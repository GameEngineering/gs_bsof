...
typedef enum
{
    BSF_BULLET_PLAYER = 0x00,
    BSF_BULLET_ENEMY
} bsf_bullet_type;

typedef struct
{
    bsf_bullet_type type;
} bsf_component_bullet_t;
...
ECS_COMPONENT_DECLARE(bsf_component_bullet_t);
...
typedef struct 
{
    bsf_component_renderable_t renderable;
    bsf_component_transform_t transform;
    bsf_component_physics_t physics; 
    bsf_component_timer_t timer;                // Kill timer
    bsf_component_bullet_t bullet;
} bsf_bullet_t; 

GS_API_DECL void bsf_bullet_create(struct bsf_t* bsf, ecs_world_t* world, bsf_bullet_type type, const gs_vqs* xform, gs_vec3 velocity);   // Create single bullet
GS_API_DECL void bsf_bullet_system(ecs_iter_t* it);                                             // System for updating bullets
...
typedef gs_camera_t bsf_camera_t; 
    ...
    { 
        ...
        switch (bsf->state)
        {
            case BSF_STATE_PLAY:
            {
                ...
                const gs_mat4 vp = gs_camera_get_view_projection(&bsf->scene.camera, fbs.x, fbs.y); 
...
#define ECS_REGISTER_COMP(T)\
    ...
    ECS_REGISTER_COMP(bsf_component_bullet_t); 
    ...
    ECS_SYSTEM(
        ...
        bsf_bullet_system, 
        EcsOnUpdate,
        bsf_component_renderable_t, bsf_component_transform_t, bsf_component_physics_t, bsf_component_timer_t, bsf_component_bullet_t
    );
...
GS_API_DECL void bsf_physics_debug_draw_system(ecs_iter_t* it)
{
    ...
    gsi_box(gsi, 0.f, sz / 2.f, 0.f, sz + 1.f, sz / 2.f, sz + 1.f, 0, 255, 0, 255, GS_GRAPHICS_PRIMITIVE_LINES);
} 
...
GS_API_DECL void bsf_camera_init(struct bsf_t* bsf, gs_camera_t* camera)
{
    *camera = gs_camera_perspective();
}

GS_API_DECL void bsf_camera_update(struct bsf_t* bsf, gs_camera_t* camera) 
{
    const float dt = gs_platform_delta_time();
    const bsf_component_transform_t* ptc = ecs_get(bsf->entities.world, bsf->entities.player, bsf_component_transform_t);
	bsf_camera_t* c = camera; 
    ...
    switch (room->movement_type)
    {
        ...
        { 
            gs_vec3 backward = gs_quat_rotate(ptc->xform.rotation, gs_vec3_scale(GS_ZAXIS, 1.f));
            ...
            gs_vec3 euler = gs_quat_to_euler(&ptc->xform.rotation); 
            euler.z = 0.f;
            
            c->transform.rotation = gs_quat_slerp(c->transform.rotation, ptc->xform.rotation, rlerp);
            // c->transform.rotation = gs_quat_slerp(c->transform.rotation, gs_quat_from_euler(euler.y, euler.x, euler.z), rlerp);
...
GS_API_DECL void bsf_mob_system(ecs_iter_t* it)
{
    ...
    for (uint32_t i = 0; i < it->count; ++i)
    {
        ...
        const gs_vec3 dir = gs_vec3_norm(gs_vec3_add(gs_vec3_sub(*ppos, tc->xform.translation), gs_vec3_scale(spread, 0.8f)));
        ...
        gsi_defaults(gsi);
        gsi_depth_enabled(gsi, true);
        gsi_camera(gsi, &bsf->scene.camera, (u32)fbs.x, (u32)fbs.y);
        ...
        // Fire
        const uint32_t m = (uint32_t)gc->time % 100;
        if (m == 0 && gs_rand_gen_long(&bsf->run.rand) % 2)
        { 
            // Fire bullet using forward of player transform
            gs_vec3 forward = dir; 
            gs_vqs xform = (gs_vqs){
            ...
            bsf_bullet_create(bsf, it->world, BSF_BULLET_ENEMY, &xform, forward);
        }
...
GS_API_DECL void bsf_bullet_create(struct bsf_t* bsf, ecs_world_t* world, bsf_bullet_type type, const gs_vqs* xform, gs_vec3 velocity)
{
    ...
    ecs_set(world, b, bsf_component_bullet_t, {.type = type});
}

GS_API_DECL void bsf_bullet_system(ecs_iter_t* it)
{
    bsf_t* bsf = gs_user_data(bsf_t); 
    const float t = gs_platform_elapsed_time();
    const float dt = gs_platform_delta_time(); 
    ...
    bsf_component_bullet_t* bca = ecs_term(it, bsf_component_bullet_t, 5);
    ...
    for (uint32_t i = 0; i < it->count; ++i)
    {
		...
		bsf_component_bullet_t* bc = &bca[i];
            ...
            { 
                ...
                { 
                    ...
                    for (uint32_t m = 0; m < gs_dyn_array_size(room->mobs); ++m) 
                    {
                        ...
                        gs_cylinder_t* c = &pc->collider.shape.cylinder; 
                        ...
                        gs_vqs xf0 = gs_vqs_absolute_transform(&phys->collider.xform, &tform->xform);
                        gs_vqs xf1 = gs_vqs_absolute_transform(&pc->collider.xform, &tc->xform);

                        switch (phys->collider.type)
                        {
                            case BSF_COLLIDER_AABB: 
                            { 
                                gs_aabb_vs_cylinder(&phys->collider.shape.aabb, &xf0, c, &xf1, &res);
                            } break;

                            case BSF_COLLIDER_SPHERE: 
                            { 
                                gs_sphere_vs_cylinder(&phys->collider.shape.sphere, &xf0, c, &xf1, &res);
                            } break;

                            case BSF_COLLIDER_CONE: 
                            { 
                                gs_cone_vs_cylinder(&phys->collider.shape.cone, &xf0, c, &xf1, &res);
                            } break;
                        }
                ...
                case BSF_BULLET_ENEMY:
                {
                    // Check collision against player
                    gs_cylinder_t* c = &pc->collider.shape.cylinder; 

                    bsf_component_transform_t* tform = ecs_get(bsf->entities.world, bsf->entities.player, bsf_component_transform_t);
                    bsf_component_physics_t* phys = ecs_get(bsf->entities.world, bsf->entities.player, bsf_component_physics_t); 
                    gs_contact_info_t res = {0};
...
{ 
    ...
    ecs_set(bsf->entities.world, p, bsf_component_health_t, {0});
}
...
{ 
    ...
    if (gs_platform_key_down(GS_KEYCODE_LEFT_SHIFT) || gs_platform_key_down(GS_KEYCODE_B)) {
        mod = 2.f;
    }
    ...
    { 
        ...
        switch (room->movement_type)
        {
            ...
            { 
                ...
                v = gs_vec3_scale(gs_vec3_norm(gs_quat_rotate(tc->xform.rotation, v)), pc->speed * dt); 

                // Forward impulse
                gs_vec3 forward = gs_quat_rotate(tc->xform.rotation, gs_vec3_scale(GS_ZAXIS, -1.f)); 

                pc->velocity = gs_vec3_scale(gs_vec3_norm(forward), pc->speed * dt);
                ...
                av = gs_vec3_scale(gs_vec3_norm(av), 1.2f);
        ...
        // Fire
        const uint32_t m = (uint32_t)gc->time % 20;
        if (gc->firing && m == 0)
        { 
            // Fire bullet using forward of player transform
            gs_vec3 cam_forward = gs_vec3_scale(gs_vec3_norm(gs_camera_forward(&bsf->scene.camera)), 100.f);
            ...
            bsf_bullet_create(bsf, it->world, BSF_BULLET_PLAYER, &xform, forward);
        }

        if (gc->firing) {
            gc->time += 1 * psc->fire_rate;
        }
    } 
}
...
static void bsf_game_level_gen(struct bsf_t* bsf)
{
    ...
    */ 
    ...
    // Generate a mob in the center of the play area

    room = gs_slot_array_iter_getp(bsf->run.rooms, bsf->run.room_ids[start_room]); 
    room->movement_type = BSF_MOVEMENT_FREE_RANGE; 

    gs_byte_buffer_readc(&buffer, uint16_t, ct);
    for (uint32_t i = 0; i <  ct; ++i)
    {
        bsf_shape_type type = 0;;
        ...
        switch (brush.type)
        {
            ...
            {
                type = BSF_SHAPE_CONE;
                // ecs_entity_t e = bsf_item_create(bsf, &brush.xform);
                // ecs_entity_t e = bsf_mob_create(bsf, &brush.xform, type);
                // gs_dyn_array_push(room->mobs, e); 
            } break;
        } 
...
{ 
    ...
    if (gs_platform_key_pressed(GS_KEYCODE_F1)) {
        bsf->dbg = !bsf->dbg;
        if (bsf->dbg) bsf->scene.camera.transform.rotation = gs_quat_default();
    }
    ...
    gs_gui_begin_window_ex(gui, "#ui_canvas", gs_gui_rect(0, 0, 0, 0), NULL, opt);
    {
        ...
        for (uint32_t i = 0; scls[i].can; ++i)
        {
            ...
            gs_mat4 vp = gs_camera_get_view_projection(&bsf->scene.camera, fbs.x, fbs.y);
            gs_vec4 p4 = gs_v4(pos.x, pos.y, pos.z, 1.f);
            ...
            p4.x = p4.x * fbs.x;
            p4.y = gs_map_range(1.f, 0.f, 0.f, 1.f, p4.y) * fbs.y;
            ...
            switch (scls[i].can)
            {
                case 1: 
                {
                    gs_gui_draw_box(gui, gs_gui_rect(p4.x - sz * 0.5f, p4.y - sz * 0.5f, sz, sz), (int16_t[]){w, w, w, w}, GS_COLOR_GREEN);
                } break;

                case 2:
                {
                    float lw = sz * 0.1f;
                    gs_gui_draw_rect(gui, gs_gui_rect(p4.x - lw * 0.5f, p4.y - sz * 0.5f, lw, sz), GS_COLOR_GREEN); // UD
                    gs_gui_draw_rect(gui, gs_gui_rect(p4.x - sz * 0.5f, p4.y, sz, lw), GS_COLOR_GREEN); // LR
                } break;
            }
    ...
    { 
        ...
        gs_gui_begin_window(gui, "#dbg", gs_gui_rect(100, 100, 350, 200));
        {
            ...
            if (gs_gui_begin_treenode(gui, "#player"))
            {
                ...
                GUI_LABEL("speed: %.2f", psc->speed);
                GUI_LABEL("fire_rate: %.2f", psc->fire_rate);
                GUI_LABEL("damage: %.2f", psc->damage);
                GUI_LABEL("range: %.2f", psc->damage);
                GUI_LABEL("shot_speed: %.2f", psc->shot_speed);
...
GS_API_DECL void bsf_editor_scene_cb(gs_gui_context_t* ctx, gs_gui_customcommand_t* cmd)
{
    ...
    // Draw room
	gsi_defaults(gsi);
    gsi_camera(gsi, &bsf->scene.camera, (uint32_t)vp.w, (uint32_t)vp.h); 
...
GS_API_DECL void bsf_editor(struct bsf_t* bsf)
{
	...
	gs_gui_begin_window_ex(gui, "Debug", gs_gui_rect(0, 0, 300.f, 200.f), NULL, 0x00);
    {
        ...
        if (gs_gui_combo(gui, "Camera Projection", &proj_type, items, ct, ct))
        {
            switch (proj_type)
            {
                case 0: bsf->scene.camera.proj_type = GS_PROJECTION_TYPE_ORTHOGRAPHIC; break;
                case 1: bsf->scene.camera.proj_type = GS_PROJECTION_TYPE_PERSPECTIVE; break;
            }
        }
	...
	{ 
        ...
        if (gui->hover_root == gs_gui_get_current_container(gui))
        {
            ...
            bsf->scene.camera.ortho_scale -= wheel.y;
        }
...
GS_API_DECL void bsf_editor_camera_update(struct bsf_t* bsf, gs_camera_t* camera)
{ 
    ...
    switch (camera->proj_type)
    {
        ...
        case  GS_PROJECTION_TYPE_PERSPECTIVE:
        {
            ...
            if (gs_platform_key_down(GS_KEYCODE_D)) vel = gs_vec3_add(vel, gs_camera_right(camera)); 
            gs_vec2 wheel = gs_platform_mouse_wheelv();
            cam_speed = gs_clamp(cam_speed + wheel.y, 0.01f, 50.f);
        } break;
    }

    camera->transform.position = gs_vec3_add(camera->transform.position, gs_vec3_scale(gs_vec3_norm(vel), dt * cam_speed * mod));
}
